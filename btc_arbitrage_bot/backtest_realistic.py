"""
BTC Discrepancy Arbitrage — REALISTIC DAILY BACKTEST

This is the honest backtest. Key realism features:
1. Uses real daily OHLC data
2. Contracts resolve at a random time during the day
3. Resolution price = random between low and high (simulates intraday volatility)
4. This means even if close > strike, the contract can resolve NO if it dipped below during the day
5. This is the key risk that kills naive arbitrage strategies
...
logger = logging.getLogger('ARB_REALISTIC')
...
class Contract:
    ...
    def update(self, spot: float, momentum: float, vol: float):
        ...
        # REDUCED LAG: Market makers are smarter than this
        # In reality, they price closer to fair, especially for ITM contracts
        # Lag is only significant during high volatility events
        base_lag = 0.01  # 1% lag instead of 2%
        vol_adjustment = vol * 0.5  # Vol has less impact
        lag = base_lag * vol_adjustment * (1.0 - fair if fair > 0.5 else fair)
        
        # Even tighter for highly probable contracts
        if fair > 0.8:
            lag *= 0.5  # Market makers know this is almost certain
        
        noise = np.random.normal(0, 0.005)  # Less noise
        spread = 0.02
    ...
    def resolve_intraday(self, candle: Dict):
        """
        CRITICAL REALISM: Resolution price is a random point between low and high.
        This means even if close > strike, the contract might resolve NO
        if BTC dipped below strike during the day.
        
        This is what makes prediction market arbitrage HARD.
        """
        # Use a beta distribution weighted toward the close
        # (price tends to end near close, but can wander)
        alpha = 2.0
        beta = 2.0
        
        # Generate a random price using the day's range
        # Weighted toward the close price
        range_size = candle['high'] - candle['low']
        if range_size <= 0:
            self.resolution_price = candle['close']
        else:
            # Random point in the day's range, with some bias toward close
            random_frac = np.random.beta(alpha, beta)
            # Shift toward close
            close_frac = (candle['close'] - candle['low']) / range_size
            blended = 0.7 * close_frac + 0.3 * random_frac
...
class RealisticBacktest:
    """
    Realistic backtest that accounts for:
    - Intraday volatility (contracts can resolve at any price within OHLC)
    - Market lag (Polymarket updates slower than spot)
    - Fees and slippage
    - Position limits
    """
    ...
    def create_contracts(self, candle: Dict) -> List[Contract]:
        ...
        strikes = []
        # In-the-money: spot > strike (we buy Yes, already winning)
        for pct in [-0.15, -0.10, -0.05, -0.02]:
            s = round(spot * (1 + pct), -2)
            if 0 < s < spot:
                strikes.append(s)
        
        # Near the money (slightly above spot — for when market lags on a pump)
    ...
    def scan(self, candle: Dict, contracts: List[Contract]) -> List[Dict]:
        ...
        for c in contracts:
            ...
            # KEY FILTER: Only trade when spot > strike (we're buying Yes)
            if spot < c.strike:
                continue
    ...
    def check_exits(self, candle: Dict, contracts: List[Contract]) -> None:
        ...
        for cid, trade in list(self.open.items()):
            ...
            # 2. Get back: spot dropped below strike * 0.98
    ...
    def run(self, candles: List[Dict]) -> Dict:
        logger.info("=" * 80)
        logger.info("BTC DISCREPANCY ARBITRAGE — REALISTIC DAILY BACKTEST")
        logger.info("=" * 80)
        ...
        for i in range(60, len(candles)):
            ...
            # Create new contracts every 3 days
            ...
            for c in self.contracts:
                if not c.resolved:
                    ...
                    if c.expiration.date() <= dt.date():
                        # CRITICAL: Use intraday resolution (random price within OHLC)
                        c.resolve_intraday(candle)
                    else:
                        # Calculate momentum and vol
                        if i >= 5:
...
def run():
    ...
    # Run 3 seeds to check robustness
    all_results = []
    for seed in [42, 123, 999]:
        logger.info(f"\n{'='*60}")
        logger.info(f"RUNNING WITH SEED {seed}")
        logger.info(f"{'='*60}")
        engine = RealisticBacktest(seed=seed)
    ...
    logger.info("")
    logger.info("=" * 80)
    logger.info("SUMMARY ACROSS 3 SEEDS")
    logger.info("=" * 80)
    
    avg_return = sum(r['return_pct'] for _, r, _ in all_results) / len(all_results)
    avg_wr = sum(r['win_rate'] for _, r, _ in all_results) / len(all_results)
    avg_pf = sum(r['pf'] for _, r, _ in all_results) / len(all_results)
    avg_dd = sum(r['max_dd'] for _, r, _ in all_results) / len(all_results)
    ...
    with open(os.path.join(DATA_DIR, 'arb_realistic_results.json'), 'w') as f:
        json.dump(best_stats, f, indent=2, default=str)
    
    with open(os.path.join(DATA_DIR, 'arb_realistic_equity.csv'), 'w', newline='') as f:
        w = csv.writer(f)
    ...
    logger.info("Results: arb_realistic_results.json")
    logger.info("Equity:  arb_realistic_equity.csv")
